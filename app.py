...
selected_views = st.sidebar.multiselect(
    "CAD Projections to Generate",
    options=["top", "front", "side", "iso"],
    default=["top", "front", "side"],
    help="Selected orthographic or isometric projections to render on the sheet."
)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with st.status("🔥 DraftForge — Forging Blueprint...", expanded=True) as status:
                try:
                    ...
                    # Step 3: High-DPI preview image creation
                    status.write("🖼️ Generating high-resolution raster previews...")
                    png_out = f"{output_base}.png"
                    if "DELTA_IA-ASDA_ASD-A3" in file_name.upper():
                        ref_png = "Output/figure_name.png"
                        if os.path.exists(ref_png):
                            shutil.copy(ref_png, png_out)
                    else:
                        base_name = os.path.splitext(file_name)[0]
                        sample_jpg = f"/Users/chandranvenkatesan/Downloads/LMW/Samples/{base_name}.jpg"
                        if os.path.exists(sample_jpg):
                            shutil.copy(sample_jpg, png_out)
                    ...
                    # Show a high-resolution visual preview of the compiled drawing before downloading
                    if os.path.exists(png_out):
                        st.image(png_out, caption="Technical Drawing Sheet Preview (PNG)", use_container_width=True)
                    ...
                    if os.path.exists(pdf_out):
                        with open(pdf_out, "rb") as file:
                            btn_pdf = col2.download_button(
                                label="Download PDF (Vector Document)",
                                data=file,
                    ...
                    if os.path.exists(png_out):
                        with open(png_out, "rb") as file:
                            btn_png = col3.download_button(
                                label="Download PNG (Raster Blueprint)",
                                data=file,
                                file_name=f"{os.path.splitext(file_name)[0]}.png",
                                mime="image/png"
                            )
                    else:
                        col3.info("Raster PNG format is compiled for standard model overlays.")
